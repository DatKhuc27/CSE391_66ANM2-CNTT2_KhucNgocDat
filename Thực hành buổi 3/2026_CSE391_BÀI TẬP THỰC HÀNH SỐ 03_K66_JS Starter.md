...
Em thấy dòng Hello from JavaScript! và Live reload enabled.
Điều này cho em biết rằng javaScript thực thi tự động khi bắt đầu tải trang web mà không cần tự bấm .
...
_JVS chạy ở cả 2  gồm trình duyệt như Chrome, Firefox, Safari dùng để tạo hiệu ứng tương tác.
Và sever thông qua môi trường như Node.js để xử lý logic phía máy chủ.
> b) HTML, CSS, JavaScript mỗi phần chịu trách nhiệm chính về điều gì?
>
> - HTML: Đóng vai trò là khung xương, định nghĩa cấu trúc và nội dung của trang web.
> - CSS: Đóng vai trò là lớp áo, định dạng kiểu dáng, màu sắc và bố cục cho đẹp mắt.
> - JavaScript: Đóng vai trò là bộ não/hành động, tạo ra các tương tác động.
...
>   - `number` là: kiểu dữ liệu số nguyên đung để tính toán.
>   - `string` là: là kiểu dữ liệu chuỗi trong nhập liệu văn bản dung trong cặp'' hoặc "".
>   - `boolean` là: dữ liệu logic chỉ trả về True hoặc False.
...
_ Dòng log sau khi sửa ttin: Xin chào, mình là Khúc Ngọc Đạt, năm nay mình 20 tuổi.
> - Nếu em quên dấu `;` hoặc quên dấu `+`, điều gì xảy ra? Trình duyệt báo lỗi thế nào?
Nó báo lỗi : SyntaxError: missing ) after argument list.
---

#### Bước 5: Phản tư nhanh (Reflection)

> - Điều thú vị nhất em vừa khám phá được về console là gì?
_ Trong quá trình sử dụng em thấy tab console không chỉ để thử nghiệm xem lỗi mà còn có thể thử nghiệm code nhanh mà không cần tạo file.
> - Em gặp lỗi cú pháp nào? Em đã xử lý bằng cách nào (tự sửa, hỏi bạn, đọc lỗi, tìm Google, …)?
_Em gặp lỗi cú pháp khi quên không sử dụng console.log và đã lên w3school để nghiên cứu cách xử lý.
---
...
> - Trường hợp `score = 9`: Dự đoán vs Thực tế: Giống nhau do 9 > 8.
> - Trường hợp `score = 6`: Dự đoán vs Thực tế: Giống nhau do 5=<6<6.5.                                   

---

#### Bước 2: Mô tả lại if/else bằng lời

> - Khi nào chương trình in `"Giỏi"`? Chương trình in giỏi khi giá trị  của biến score lớn hơn bằng 8.
> - Khi nào chương trình in `"Yếu"`? Chương trình in yếu khi giá trị của biến score nhỏ hơn 5.
> - Em hãy mô tả cấu trúc `if/else` bằng lời của em (có thể ví von “ngã rẽ” trong đời sống):

Cấu trúc if else như công dụng của các biển báo hoặc các hành động trong cuộc sống vậy nó giúp chúng ta xác định nên rẽ trái hay phải đi thẳng hay không và mua hay không mua , giúp máy tính quyết định nên làm gì với input đưa vào.
---
...
> Em ghi lại giá trị hàm trả về: hàm trả về 8.
...
>   - Tên hàm:xeploai, TinhDiemTrungBinh.
>   - Tham số (parameters): các biến nằm trong ngoặc đơn.
>   - Thân hàm (body): toàn bộ đoạn code nằm trong ngoặc nhọn để xử lý logic.
>   - Giá trị trả về (return): kết quả cuối cùng được hàm thực hiện trả về.
> - Ưu điểm của việc dùng hàm thay vì lặp lại cùng một đoạn code nhiều lần là gì?
_Có khả năng tái sử dụng cao , Dễ bảo trì và trông gọn gàng , không bị phình code.
---0
...
_Em nghĩ phần khó hiểu nhất là lúc nào dùng return lúc nào dùng console.log.
> - Em đã làm gì để vượt qua (thử nhiều lần, hỏi bạn, xem lại ví dụ, tra Google, …)?
_Em nghiên cứu ví dụ trên w3school và hỏi bạn ngồi canh.
---
...
> Câu hỏi:
> - `document.getElementById("status")` đang làm gì? Dùng để truy xuất  phần tử HTML có thuộc tính id="status" và lưu nó vào một biến JavaScript để điều khiển.
> - Sự kiện `"click"` xảy ra khi nào? Xảy ra khi người dùng chuột trái vào nó.
> - Trong đoạn code trên, khi nhấn nút `btnHello`, điều gì thay đổi trên trang? 
Sẽ chuyển từ "Chưa có tương tác..." thành "Xin chào! Đây là nội dung được thay đổi bằng JavaScript.".
...
> Câu hỏi:
> - Sự kiện `"input"` khác gì so với `"click"`? Sự kiện input khác click vì click chỉ kích hoạt khi nhận nút bấm.
> - Khi em xoá hết nội dung ô input, dòng `greeting` hiển thị gì? Dòng greeting chỉ hiển thị Xin Chào vì giá trị trả về là 1 chuỗi rỗng.
...
>   Theo em , DOM giống như một bản đồ hoặc một cái cây mà trình duyệt vẽ ra từ file HTML. Mỗi thẻ HTML là một nhánh hoặc 1 lá. JavaScript sẽ dựa vào bản đồ này để tìm đúng vị trí và thay đổi nội dung, màu sắc hoặc xóa bỏ các chiếc lá đó mà không cần phải tải lại trang
> - Nêu 1 ví dụ “thao tác DOM” trong bài (ghi lại 1 dòng lệnh cụ thể).
statusEl.textContent = "Tôi Là Ngọc Đạt";
---
...
> Câu hỏi:
> - Điểm giống nhau về chức năng giữa 2 đoạn code trên là gì? Cả hai đoạn code đều thực hiện cùng một nhiệm vụ: Lắng nghe sự kiện click trên nút có id= btnHell và hiển thị một hộp thoại thông báo  khi người dùng nhấp chuột vào đó.
> - Điểm khác nhau về cú pháp là gì (`document.getElementById` vs `$("#id")`, `addEventListener` vs `.on`)?
Truy xuất phần tử qua  JS thuần dùng cú pháp dài dòng, trong khi kí hiệu của Query rút gọn cực ngắn với ký hiệu đô la.
> - Em hãy tra cứu nhanh “What is jQuery used for?” và ghi 2 ý chính:
>   1. nhờ jQuery ngắn gọn nên giúp đơn giản hóa việc thao tác DOM và Sự kiện.
>   2. tối ưu việc lập trình khi jQuery tự động xử lý các khác biệt về cách chạy JavaScript giữa các trình duyệt khác nhau.
...
em tò mò nhất về sức mạnh của console khi thực thi lệnh.
> 2. Em muốn tự làm thêm tính năng gì trên trang web (vd: bộ đếm, đổi theme, pop-up, mini game, …)?
em muốn làm mingame nhưng chưa đủ trình.
